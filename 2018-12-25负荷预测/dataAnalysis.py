...
#   plot_dif(data)
# -----------------------------绘制气候数据图.
#   plot_temperature(**para)
#   plot_humidity(**para)
#   accTem_effect(dataSet)
# -----------------------------负荷缺失值检测.
#   detect_missing_value(dataSet, genre='1')
# -----------------------------聚类分析.
#   cluster_analysis(dataSet)

def createHalfLoadSet(dataSet):
    """
    生成以半小时为单位的负荷数据.
    ...
    tem = dataSet.iloc[0].t_10
    rh = dataSet.iloc[0].rh_10
    ...
    for index, row in dataSet.iterrows():
        ...
        if half != row['Half']:
            ...
            Year.append(year)
            Value.append(value)
            F_half.append(f_half)
            F_day.append(f_day)
            F_week.append(f_week)
    ...
    HalfLoadSet.insert(0, 'rh_10', np.array(rh_10))
    HalfLoadSet.insert(0, 't_10', np.array(t_10))
    HalfLoadSet.insert(0, 'Season', np.array(Season))
    HalfLoadSet.insert(0, 'isHoliday', np.array(isHoliday))
    HalfLoadSet.insert(0, 'isWorkday', np.array(isWorkday))
    HalfLoadSet.insert(0, 'dayOfWeek', np.array(dayOfWeek))
    HalfLoadSet.insert(0, 'F_week', np.array(F_week))
    HalfLoadSet.insert(0, 'F_day', np.array(F_day))
    HalfLoadSet.insert(0, 'F_half', np.array(F_half))
    HalfLoadSet.insert(0, 'Value', np.array(Value))
    HalfLoadSet.insert(0, 'Half', np.array(Half))
    HalfLoadSet.insert(0, 'Hour', np.array(Hour))
    HalfLoadSet.insert(0, 'Day', np.array(Day))
    HalfLoadSet.insert(0, 'Month', np.array(Month))
    HalfLoadSet.insert(0, 'Year', np.array(Year))
    HalfLoadSet.insert(0, 'Data_Time', np.array(Data_Time))

    # supplementaryData_2 = pd.read_csv('Supplementary_data_2.csv')
    # for index, row in HalfLoadSet.iterrows():
    #     year = row['Year']
    #     month = row['Month']
    #     day = row['Day']
    ...
    # HalfLoadSet.insert(8, 'dayOfWeek', np.array(dayOfWeek))
    # HalfLoadSet.insert(10, 'isHoliday', np.array(isHoliday))

    HalfLoadSet.to_csv('HalfLoadSet.csv', index=False)

    end = time.clock()
    print(end - start)  # 41s


def createHourLoadSet(dataSet):
    """
    生成以小时为单位的负荷数据.
    暂未加入 F_half F_day F_week 3列特征.
    ...
    HourLoadSet.insert(0, 'Value3', np.array(Value3))
    HourLoadSet.insert(0, 'Value2', np.array(Value2))
    HourLoadSet.insert(0, 'Value1', np.array(Value1))
    HourLoadSet.insert(0, 'Value', np.array(Value))
    HourLoadSet.insert(0, 'rh_10', np.array(rh_10))
    HourLoadSet.insert(0, 't_10', np.array(t_10))
    HourLoadSet.insert(0, 'Season', np.array(Season))
    HourLoadSet.insert(0, 'isHoliday', np.array(isHoliday))
    HourLoadSet.insert(0, 'isWorkday', np.array(isWorkday))
    HourLoadSet.insert(0, 'dayOfWeek', np.array(dayOfWeek))
    HourLoadSet.insert(0, 'Value32', np.array(Value32))
    HourLoadSet.insert(0, 'Value31', np.array(Value31))
    HourLoadSet.insert(0, 'Value23', np.array(Value23))
    HourLoadSet.insert(0, 'Value22', np.array(Value22))
    HourLoadSet.insert(0, 'Value21', np.array(Value21))
    HourLoadSet.insert(0, 'Value11', np.array(Value11))
    HourLoadSet.insert(0, 'Hour', np.array(Hour))
    HourLoadSet.insert(0, 'Day', np.array(Day))
    HourLoadSet.insert(0, 'Month', np.array(Month))
    HourLoadSet.insert(0, 'Year', np.array(Year))
    HourLoadSet.insert(0, 'Data_Time', np.array(Data_Time))

    # supplementaryData_2 = pd.read_csv('Supplementary_data_2.csv')
    # for index, row in HourLoadSet.iterrows():
    #     year = row['Year']
    #     month = row['Month']
    #     day = row['Day']
    #     a = supplementaryData_2[supplementaryData_2['Year']==year]
    ...
    HourLoadSet.to_csv('HourLoadSet2018.csv', index=False)
...
def plot_half_week(dataSet, month=1, day=1, eid=1):
    """
    绘制消纳的周负荷曲线, 精度为半小时.
...
def plot_hour_year(dataSet):
    pass


def plot_hour_month(dataSet):
    pass


def plot_hour_week(dataSet):
    pass


def plot_hour_day(dataSet):
    pass
...
def plot_holiday():
    ...
    # data1 = dataSet[(dataSet.isHoliday == 1) & (dataSet.Year == 2017)]
    # data2 = dataSet.loc[data1.index.values - 17520]
    # plt.plot(range(len(data1)), data1.Value, label='Holiday of 2017')
    # plt.plot(range(len(data2)), data2.Value, label='Not holiday of 2016')
    # plt.legend()
    # plt.show()

    dataSet = pd.read_csv('DayLoadSet.csv')
    data1 = dataSet[(dataSet.isHoliday == 1) & (dataSet.Year == 2017)]
    data2 = dataSet.loc[data1.index.values - 365]
    plt.plot(range(len(data1)), data1.Value, label='Holiday of 2017')
    plt.plot(range(len(data2)), data2.Value, label='Not holiday of 2016')
    plt.plot(range(len(data2)), data1.Value.values - data2.Value.values, marker='.', lw=4, label='Differences')
    plt.plot(range(len(data2)), len(data1) * [0], ls='--')
    plt.legend()
    plt.show()

    # dataSet = pd.read_csv('DayLoadSet.csv')
    # data1 = dataSet[(dataSet.isHoliday == 1) & (dataSet.Year == 2016)]
    # data2 = dataSet.loc[data1.index.values + 365]
    # plt.plot(range(len(data1)), data1.Value, label='Holiday of 2016')
    # plt.plot(range(len(data2)), data2.Value, label='Not holiday of 2017')
    # plt.plot(range(len(data2)), data1.Value.values-data2.Value.values, marker='.', lw=4, label='Differences')
    # plt.plot(range(len(data2)), len(data1)*[0], ls='--')
    # plt.legend()
    # plt.show()
...
def accTem_effect():
    ...
    dataSet = pd.read_csv('DayLoadSet.csv')
    data = dataSet[(dataSet.Season == 2) | (dataSet.Season == 3)]
    Value2 = data.Value2.values
    Tem_max = data.Tem_max.values
    Value2 = (Value2 - Value2.min()) / (Value2.max() - Value2.min())
    Tem_max = (Tem_max - Tem_max.min()) / (Tem_max.max() - Tem_max.min())
    plt.plot(Value2, label='Value2')
    plt.plot(Tem_max, label='Tem_max')
    plt.plot([len(Value2) / 2, len(Value2) / 2], [Value2.max() + 0.3, Value2.min() - 0.3], ls='--')
    plt.title('The accumulated temperature effect')
    plt.legend()
    plt.show()
    pass
